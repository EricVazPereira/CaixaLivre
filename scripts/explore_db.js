...
function cols(db, table) {
  return new Promise((res, rej) => {
    const sql = `SELECT f.RDB$FIELD_NAME, t.RDB$TYPE_NAME, f.RDB$NULL_FLAG
...
function sample(db, table) {
  return new Promise((res, rej) => {
    db.query(`SELECT FIRST 2 * FROM ${table}`, [], (err, rows) => {
      if (err) rej(err); else res(rows);
    });
  });
}
...
Firebird.attach(opt, async (err, db) => {
  ...
  for (const t of ['PRODUTO', 'CONTA', 'PAGAMENTO', 'TIPO_PAGTO', 'ESTOQUE']) {
    console.log('\n=== ' + t + ' ===');
    try {
      ...
      if (s.length > 0) {
        console.log('  [amostra]', JSON.stringify(s[0], null, 2).slice(0, 500));
      }
    } catch (e) {
