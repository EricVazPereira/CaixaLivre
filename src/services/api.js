 ...
 */
export async function fecharComanda({ subtotal, total, barcode, forma_pagamento, cpf = '' }) {
  const res = await fetch(`${BASE}/historico/fechar-comanda`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ subtotal, total, barcode, forma_pagamento, cpf, discount: '0', add_service: '' }),
  })
  const data = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(data.erro || 'Erro ao fechar comanda no ERP')
  return data
}
...
export async function testarBalanca() {
  try {
    ...
    const data = await res.json().catch(() => ({}))
    return { habilitada: data.habilitada ?? true, ok: data.ok === true }
  } catch {
    return { habilitada: true, ok: false }
  }
}
...
export async function buscarConfiguracaoBalanca() {
  try {
    ...
    const data = await res.json().catch(() => ({}))
    return { habilitada: data.habilitada ?? true }
  } catch {
